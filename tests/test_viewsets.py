...
class ViewSetTestCase(TestCase):
    ...
    def test_get_view_name(self):
        ...
        viewset.suffix = "list"
        self.assertEqual(
            viewset.get_view_name(),
            viewset.view_name_prefix + ' ' + viewset.suffix
        )
    ...
    def test_apply_pagination(self):
        ...
        self.assertEqual(response.data['links']['first'], 'http://testserver/tests?page[size]=25&page[number]=1')
        self.assertEqual(response.data['links']['next'], 'http://testserver/tests?page[size]=25&page[number]=2')
        self.assertEqual(response.data['links']['prev'], None)
        self.assertEqual(response.data['links']['last'], 'http://testserver/tests?page[size]=25&page[number]=4')
    ...
    def test_apply_pagination_page_2(self):
        ...
        self.assertEqual(response.data['meta']['num_pages'], 4)
        self.assertEqual(response.data['links']['first'], 'http://testserver/tests?page[size]=25&page[number]=1')
        self.assertEqual(response.data['links']['next'], 'http://testserver/tests?page[size]=25&page[number]=3')
        self.assertEqual(response.data['links']['prev'], 'http://testserver/tests?page[size]=25&page[number]=1')
        self.assertEqual(response.data['links']['last'], 'http://testserver/tests?page[size]=25&page[number]=4')
...
class RelationshipViewSetTestCase(TestCase):

    def test_get_related(self):
        with self.assertRaises(NotImplementedError):
            RelationshipViewSet().get_related(None, None)
